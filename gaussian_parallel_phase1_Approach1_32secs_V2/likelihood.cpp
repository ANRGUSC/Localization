...
static size_t prev_kudaTransferKernelMatrix_iter = 0;
...
void populate_likelihood(int p, int q)
{
	...
	if ((kudaTransferKernelMatrix_iter - prev_kudaTransferKernelMatrix_iter) >= 3000)
	{
		kernelDriver(prev_kudaTransferKernelMatrix_iter);
		prev_kudaTransferKernelMatrix_iter += 3000;
	}
}
